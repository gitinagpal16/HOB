...
function ProductDetail({ category, product }) {
  ...
  return (
    <>
      <section className="py-8 sm:py-12 lg:py-16">
        <div className="max-w-7xl mx-auto px-5 sm:px-8">
          <div className="flex items-center gap-2 text-[11px] uppercase tracking-[0.18em] text-[#F5F1E8]/45 mb-8">
            <Link to={`/category/${category.id}`} className="hover:text-[#D3AA66] transition-colors">
              {category.name}
            </Link>
            <span>/</span>
            <span className="text-[#D3AA66]">{product.name}</span>
          </div>
          ...
          <div className="grid lg:grid-cols-[minmax(0,1.08fr)_minmax(360px,0.92fr)] gap-8 lg:gap-14 items-start">
            <div className="grid grid-cols-[72px_minmax(0,1fr)] sm:grid-cols-[92px_minmax(0,1fr)] gap-4">
              ...
              >
                <ProductImage src={activeImage} alt={`${product.name} selected view`} className="transition-transform duration-500 group-hover:scale-[1.015]" />
            ...
            <div className="lg:pt-3">
              <p className="text-[10px] uppercase tracking-[0.35em] text-[#D3AA66] mb-4">{category.name}</p>
              ...
              <p className="text-[#F5F1E8]/65 text-sm sm:text-base leading-7 max-w-xl">
                {product.description || "Add the product description here. This space is ready for the final product story, materials, comfort features and intended salon use."}
              </p>

              <div className="mt-8 border-y border-[#B4863C]/20">
                <details className="group border-b border-[#B4863C]/20" open>
                  <summary className="list-none cursor-pointer py-5 flex items-center justify-between text-[11px] uppercase tracking-[0.2em] text-[#F5F1E8]/80">
                    Description
                    <Plus size={16} className="group-open:rotate-45 transition-transform text-[#D3AA66]" />
                  </summary>
                  <div className="pb-5 text-sm leading-7 text-[#F5F1E8]/55">
                    {product.description || `Add the detailed description for ${product.name} here.`}
                  </div>
                </details>
              ...
              <a
                href={waLink(`Hello Nagpal's House of Beauty, I'd like to enquire about ${product.name} in ${category.name}.`)}
                target="_blank"
                rel="noreferrer"
      ...
      {lightboxOpen && (
        <div className="fixed inset-0 z-[100] bg-[#080B09]/95 backdrop-blur-md flex items-center justify-center p-4 sm:p-8" data-testid="product-lightbox">
          <button
            type="button"
          ...
          <button type="button" onClick={previous} className="absolute left-3 sm:left-7 top-1/2 -translate-y-1/2 z-20 w-11 h-11 rounded-full border border-[#F5F1E8]/25 bg-[#0F1411]/70 text-[#F5F1E8] flex items-center justify-center hover:border-[#D3AA66] hover:text-[#D3AA66]" aria-label="Previous image">
            <ChevronLeft size={24} />
          </button>
          <button type="button" onClick={next} className="absolute right-3 sm:right-7 top-1/2 -translate-y-1/2 z-20 w-11 h-11 rounded-full border border-[#F5F1E8]/25 bg-[#0F1411]/70 text-[#F5F1E8] flex items-center justify-center hover:border-[#D3AA66] hover:text-[#D3AA66]" aria-label="Next image">
          ...
          <div className="w-full max-w-6xl h-full flex flex-col items-center justify-center gap-4">
            ...
            <div className="flex items-center gap-3">
              <button type="button" onClick={() => setZoom((z) => Math.max(1, +(z - 0.25).toFixed(2)))} className="w-10 h-10 border border-[#F5F1E8]/20 text-[#F5F1E8] hover:border-[#D3AA66] hover:text-[#D3AA66] flex items-center justify-center" aria-label="Zoom out"><Minus size={16} /></button>
              <span className="min-w-16 text-center text-xs text-[#F5F1E8]/70">{Math.round(zoom * 100)}%</span>
              <button type="button" onClick={() => setZoom((z) => Math.min(2.5, +(z + 0.25).toFixed(2)))} className="w-10 h-10 border border-[#F5F1E8]/20 text-[#F5F1E8] hover:border-[#D3AA66] hover:text-[#D3AA66] flex items-center justify-center" aria-label="Zoom in"><Plus size={16} /></button>
            </div>

            <div className="flex gap-2 max-w-full overflow-x-auto pb-1">
              {images.map((src, index) => (
                <button key={src} type="button" onClick={() => { setActiveIndex(index); setZoom(1); }} className={`w-16 h-16 shrink-0 bg-[#F7F5F0] border ${activeIndex === index ? "border-[#D3AA66]" : "border-[#F5F1E8]/15"}`}>
...
export default function CategoryPage() {
  ...
  const product = useMemo(
    () => products.find((p) => p.id === productId || encodeURIComponent(p.name) === productId),
    [products, productId]
  );
    ...
    return (
      <main className="min-h-[70vh] flex flex-col items-center justify-center gap-6 px-6 text-center" data-testid="category-not-found">
        <h1 className="font-display font-bold uppercase text-2xl text-[#F5F1E8]">
          {productId ? "Product not found" : "Category not found"}
        </h1>
  ...
  if (product) {
    return <main data-testid="product-detail-page"><ProductDetail category={category} product={product} /></main>;
  }
  ...
  return (
    <main data-testid="category-page">
      ...
      <section className="py-12 sm:py-16">
        <div className="max-w-7xl mx-auto px-5 sm:px-8">
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-x-5 gap-y-10" data-testid="category-products-grid">
            {products.map((p, i) => (
              <Reveal key={p.id || p.name} delay={(i % 4) * 0.06}>
                <Link
                  to={`/category/${category.id}/product/${p.id || encodeURIComponent(p.name)}`}
                ...
                >
                  <div className="relative aspect-[4/5] overflow-hidden bg-[#F7F5F0] border border-[#B4863C]/15 group-hover:border-[#B4863C]/50 transition-colors duration-500">
                    <ProductImage src={p.image} alt={p.name} className="transition-transform duration-700 group-hover:scale-[1.02]" />
                  </div>
                  <div className="pt-4 flex items-center justify-between gap-3">
                    <h3 className="font-display font-semibold text-base sm:text-lg text-[#F5F1E8] group-hover:text-[#D3AA66] transition-colors">{p.name}</h3>
                    <ChevronRight size={17} className="shrink-0 text-[#B4863C] group-hover:translate-x-1 transition-transform" />
                  </div>
                </Link>
              </Reveal>
            ))}
